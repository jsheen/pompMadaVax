...
import_input <- 1 / 10 # 1 infectious import per month on average
...
sir_rinit <- function (...) {
  c(cnt=1, S = 334, I = 30, V_S=0, V_I=0, R = 57, D=7, H = 7, W=0)
}
...
slice_design(
  center=coef(mod),
  Beta=rep(seq(from=0.001989653,to=0.01711101,length.out=20),each=1)
) -> p
plan(multisession)
foreach (
  theta=iter(p,"row"),
...
#save(p_beta, file=paste0("~/vaxMada-data/objects/data_obj/p_beta_coarse_vill2_sensL.RData"))
#save(mcap_res_beta, file="~/vaxMada-data/objects/data_obj/mcap_res_beta_coarse_vill2_sensL.RData")
## Split in half for finer grain search
slice_design(
  center=coef(mod),
  Beta=rep(seq(from=0.001989653,to=(0.01711101 / 2),length.out=50),each=1)
) -> p
plan(multisession)
foreach (
  theta=iter(p,"row"),
...
#save(p_beta, file=paste0("~/vaxMada-data/objects/data_obj/p_beta_vill2_sensL.RData"))
#save(mcap_res_beta, file="~/vaxMada-data/objects/data_obj/mcap_res_beta_vill2_sensL.RData")
