...
library(ggplot2)
load(file="~/vaxMada-data/objects/data_obj/data_vill2.RData")
load(file=paste0("~/vaxMada-data/objects/data_obj/p_beta_vill2_sensL.RData"))
load(file=paste0("~/vaxMada-data/objects/data_obj/mcap_res_beta_vill2_sensL.RData"))
...
# Initial conditions
N0 <- 421
I_init <- round(24 / mort_perc / rho_input)
S_init <- round(N0 * (1 - immune_perc)) - I_init
R_init <- N0 - S_init - I_init
sir_rinit <- function (...) {
  c(cnt=1, f_size = 0, rt=0, r0=0, eta=0, eta_best=0, S = 334, I = 30, V_S=0, V_I=0, R = 57, D=7, H = 7, W=0)
}
...
simpSims |>
  ...
  geom_line(mapping=aes(gen, y=R), col='blue') +
  geom_line(mapping=aes(gen, y=(S+I+R)), col='green') +
  geom_line(mapping=aes(gen, y=S), col='purple')
...
save(sims, file=paste0("~/vaxMada-data/objects/data_obj/sims_vill2_sensL.RData"))
save(sims_confint, file=paste0("~/vaxMada-data/objects/data_obj/sims_confint_vill2_sensL.RData"))
#load(file=paste0("~/vaxMada-data/objects/data_obj/sims_vill2_sensL.RData"))
#load(file=paste0("~/vaxMada-data/objects/data_obj/sims_confint_vill2_sensL.RData"))

# Statistics for Table 2
mean(sims$rt, na.rm=T)
sd(sims$rt, na.rm=T)
mean(sims$r0, na.rm=T)
sd(sims$r0, na.rm=T)
mean(ifelse(sims$eta < 0, 0, sims$eta), na.rm=T)
sd(ifelse(sims$eta < 0, 0, sims$eta), na.rm=T)
mean(ifelse(sims$eta_best < 0, 0, sims$eta_best), na.rm=T)
sd(ifelse(sims$eta_best < 0, 0, sims$eta_best), na.rm=T)
